ocument.addEventListener("DOMContentLoaded", function () {
  ...
  let stars = []; // Đặt là let để có thể gán lại mảng mới
  const heartStars = []; // Vẫn là const, nhưng sẽ reset nội dung
  let meteors = [];
  let fallingTexts = [];
  const clickTexts = [];
  ...
  // Tạo lại các ngôi sao của trái tim
  function resetHeartStars() {
    heartStars.length = 0; // Xóa tất cả các ngôi sao cũ
    const centerX = width / 2;
    const centerY = height / 2 + 20;
    const count = 1600; // Số lượng sao ban đầu
  ...
  // Tạo lại các ngôi sao nền
  function createBackgroundStars(count = 200) {
    stars.length = 0; // Xóa sao cũ
    for (let i = 0; i < count; i++) {
      stars.push({
        x: Math.random() * width,
        y: Math.random() * height,
  ...
  function animate() {
    ...
    // Chỉ vẽ các hiệu ứng nền khi bức thư KHÔNG hiển thị
    if (!isMessageVisible) {
      stars.forEach((star) => {
    ...
    // Click texts (tên hiện ra khi click) luôn được vẽ, kể cả khi thư đang hiện
    ...
    heartStars.forEach((star) => {
      ...
      let twinkleOpacity =
        star.brightness * (0.3 + 0.5 * Math.sin(star.twinkle));
      if (isMessageVisible) {
        twinkleOpacity *= 0.1; // Làm tim mờ hơn nữa
      }
      ctx.save();
  ...
  function startBackgroundEffects() {
    if (!fallingTextInterval)
      fallingTextInterval = setInterval(createFallingText, 2000);
  ...
  function stopBackgroundEffects() {
    ...
    meteorInterval = null;
    // Xóa sạch các đối tượng đang rơi để tránh chúng hiện lại
    meteors = [];
    fallingTexts = [];
  }
  ...
  canvas.addEventListener("click", (e) => {
    ...
    if (isMessageVisible) {
      ...
      heartClickCount = 0; // Reset bộ đếm
      resetHeartStars(); // Reset trái tim về trạng thái ban đầu
      heartScale = Math.min(width, height) * 0.015; // Reset kích thước trái tim
      createBackgroundStars(); // Tạo lại các ngôi sao nền (nếu cần)
    } else {
      heartClickCount++;
      ...
      if (heartClickCount >= requiredClicks) {
        ...
        stopBackgroundEffects(); // Dừng tất cả hiệu ứng nền
      } else {
        const centerX = width / 2,
          centerY = height / 2 + 20;
        heartScale *= 1.015; // Tăng kích thước trái tim
  ...
  window.addEventListener("resize", () => {
    ...
    createBackgroundStars(); // Tạo lại các sao nền khi resize
    resetHeartStars(); // Tạo lại các sao trái tim khi resize
  });
